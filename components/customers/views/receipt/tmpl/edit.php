...
?><div class="com-head"><h3>Cash Receipt</h3>
</div><div class="form-group">
	<form class="form-inline" method="post" name="frm" action="?com=customers&view=receipt&task=edit<?php echo $id;?>">
		<fieldset class="form">
		<div class="form grid">
			<input type="hidden" name="id" value="<?php if($row){echo $row['id'];}?>" />
			<div class="row well-sm">
				<div class="col-sm-1"><label class="control-label" for="title">Customer:</label></div><div class="col-sm-2"><input name="title" class="inputbox form-control" value="<?php if($row){echo $row['title'];}?>" readonly tabindex="-1" /></div>
