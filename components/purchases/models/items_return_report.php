...
class ModelItems_return_report extends Model{
	...
	public function getBranchPurchasesByItems(){
		$db=Core::getDBO();
		$user=Core::getUser();
		$u=$user->getUser();
		...
		$sql .= "AVG(bpa.unit_price) AS costPrice, SUM(bpa.qty_scheme) AS sum_qty, SUM(bpa.sale_price*bpa.qty_scheme) AS total ";
		//$sql .= "SUM(bpa.price - bpa.cost_price) AS margin ";
		$sql .= "FROM branch_purchase_returns AS bpr ";
		$sql .= "LEFT JOIN suppliers AS s ON (bpr.supplier_id = s.id) ";
		$sql .= "LEFT JOIN branch_purchase_return_articles AS bpa ON (bpa.return_id = bpr.id) ";
		...
		$sql .= "WHERE bpr.branch_id = {$branch_id} AND a.article_code IS NOT NULL ";
