...
$this->app->setTitle('Sale (Date wise report)');
?><div class="com-head">
...
<div class="table-responsive"><div id="search-date-range">
	...
	<table id="data-table" class="table salesReportItems">
		<thead>
			<tr>
				<th>Sr#</th><th>Date</th><th>Qty</th><th>Cost Price</th><th>Sales Price</th><th>Discount</th><th>Saving</th><th>%</th>
			</tr>
		</thead>
	...
	?>;
		...
		$.each(json_data, function(i, v) {
			...
			//console.log(vt.qty);
				var tableRow = $("#tblContentBody .sale_date").filter(function() {
					return $(this).text() == v.sale_date;
				}).closest("tr");
				if(tableRow.html()){
					...
					var new_discount = eval(vt.qty)*eval(discount);
					...
					html = html + '<td class="sale_date">'+v.sale_date+ '</td>';
					...
					html = html + '<td class="discount">'+eval(discount).toFixed(1)+'</td>';
