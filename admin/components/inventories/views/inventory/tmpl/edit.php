...
<div><form class="form-inline" method="post" name="frm" action="?com=inventories&view=inventory<?php if($this->id){echo '&id='.$this->id;} ?>">
		...
		<div class="form grid">
			...
			<div class="row well-sm">
				<div class="col-sm-1 grid-label"><label class="control-label" for="id">ID:</label></div><div class="col-sm-2"><input name="inventory_" class="inputbox form-control input-sm number" value="<?php if($this->id){echo $this->row['id'];}?>" disabled /></div>
				<div class="col-sm-1 grid-label"><label class="control-label" for="inv_date">Date:</label></div><div class="col-sm-2"><input name="inv_date" class="inputbox form-control input-sm date<?php if(!isset($this->row)){ echo '-default';}?>" value="<?php if(isset($this->row)){ echo $this->row['inv_date'];}?>" tabindex="-1" /></div>
			...
			<div class="btn-group">
				<div class="row well-sm">
					...
					<span><button id="save" name="save" class="btn btn-success" tabindex="-1"><?php 
						...
						echo $btn_title;
					?></button></span>
					<span><a href="?com=inventories&view=inventory&task=adjust&id=<?php echo $this->id; ?>" id="adjustments" class="btn btn-success" tabindex="-1">Adjustments</a></span>
				</div>
			</div>
			</fieldset>
		</fieldset></form>
